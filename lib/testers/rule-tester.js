...
class RuleTester {
    ...
    run(ruleName, rule, test) {
        ...
        function runRuleForItem(ruleName, item) {
            ...
            const schema = validator.getRuleOptionsSchema(ruleName, eslint.rules);
            ...
            validator.validate(config, "rule-tester", eslint.rules, new Environments());
            ...
            const originalGet = eslint.rules.get;

            try {
                eslint.rules.get = function(ruleId) {
                    const rule = originalGet.call(eslint.rules, ruleId);
